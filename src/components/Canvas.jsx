...
function Canvas({ isLoginOpen }) {
  ...
  return (
    <div
      className={`flex flex-col gap-8 items-center justify-center absolute top-0 transition-all duration-700 w-full h-screen ${
        isLoginOpen ? "left-full" : "left-0"
      }`}
    >
        ...
        {grid.map((row, rowIndex) => (
          <div className="flex gap-1.5">
            {row.map((col, colIndex) => (
              <div
                onClick={() => fillColor(rowIndex, colIndex)}
